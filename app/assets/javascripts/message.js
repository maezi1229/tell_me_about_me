$(function(){
  function buildHTML(message){
    if ( message.image ) {
      var html =
       `<div class="wrapper__main_chat__message-list__box" data-message-id=${message.id}>
  ...
  var reloadMessages = function() {
    ...
    var last_message_id = $('.wrapper__main_chat__message-list__box:last').data("message-id");
    $.ajax({
    ...
    .done(function(messages) {
      if (messages.length !== 0) {
        ...
        $('.wrapper__main_chat__message-list').append(insertHTML); 
        $('.wrapper__main_chat__message-list').animate({ scrollTop: $('.wrapper__main_chat__message-list')[0].scrollHeight});
      }           
    })
    .fail(function() {
